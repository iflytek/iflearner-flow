...
接下来，我们将介绍如何使用命令行工具`flow_server_cli`与`iflearner_flow_server`进行交互，以下是子命令。
```shell
...
|参数名称 |必填 |类型 |描述 |
| :---------------- | :--- | :----- | -------------------------------------------------------------- |
| --host | 是 | string | 连接`iflearner_flow_server`的ip地址  |
| --port | 是 | string | 连接`iflearner_flow_server`的端口 |
...
--8<--
client/cli/flow_server/template.zh.md
client/cli/flow_server/task.zh.md
--8<--