...
uninstall_coms(){
	echo -n "Enter the uninstall Component? flow_federate/mysql/pvc/all: "
	read COMP
	case $COMP in
	flow_federate | flow | federate | f )
		cd $DEPLOY_HOME/components/ && kubectl delete -f flow-federate-srv.yaml
		;;
  mysql | mysq | my | m )
		cd $DEPLOY_HOME/components/ && kubectl delete -f flow-federate-mysql.yaml
		;;
  pvc | p )
		cd $DEPLOY_HOME/components/ && kubectl delete -f flow-federate-pvc.yaml
		;;
	all | a)
...
install_coms(){
	echo -n "Enter the install Component? pvc/mysql/flow_federate/all: "
	read COMP
	case $COMP in
  pvc | p )
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-pvc.yaml
		;;
  mysql | mysq | my | m )
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-mysql.yaml
		sleep 10s
		init_sql
		;;
  flow_federate | flow | federate | f )
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-srv.yaml
		;;
	all | a)
		echo "install all flow Component"
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-pvc.yaml
		sleep 5s
		check_pvc flow-federate-pvc
		confirm
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-mysql.yaml
		sleep 10s
		init_sql
		check_pod flow-federate-mysql
		confirm
		cd $DEPLOY_HOME/components/ && kubectl apply -f flow-federate-srv.yaml
		sleep 5s
		;;
...
status_coms(){
	echo -n "Enter the status Component? pvc/mysql/flow_federate/all: "
	read COMP
	case $COMP in
  pvc | p )
		check_pvc flow-federate-pvc
...
function help(){
 	echo -e "Usage: flow federate deploy.sh [OPTIONS]
    Options:
